...
if ( ! prisma_core_is_woocommerce_activated() ) {
	add_action( 'activate_woocommerce/woocommerce.php', array( prisma_core_dynamic_styles(), 'delete_dynamic_file' ) );
	return;
}

/**
 * Prisma Core WooCommerce Compatibility.
 */
if ( ! class_exists( 'Prisma_Core_Woocommerce' ) ) :
	...
	class Prisma_Core_Woocommerce {
		 ...
		 *
		 * @since 1.0.0
		 * @return Prisma_Core_Woocommerce
		 */
		public static function instance() {

			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Prisma_Core_Woocommerce ) ) {
				self::$instance = new Prisma_Core_Woocommerce();
		...
		private function includes() {
			...
			require PRISMA_CORE_THEME_PATH . '/inc/compatibility/woocommerce/woocommerce-functions.php'; // phpcs:ignore
			require PRISMA_CORE_THEME_PATH . '/inc/compatibility/woocommerce/class-prisma-core-customizer-woocommerce.php'; // phpcs:ignore
		}
		...
		private function actions() {
			...
			if ( ! is_admin() ) {
				...
				// Disable Prisma Core page description.
				add_filter( 'prisma_core_page_header_description', array( $this, 'shop_remove_page_description' ) );
				...
				// Extend Prisma Core breadcrumb trail.
				add_filter( 'prisma_core_breadcrumb_trail_items', array( $this, 'breadcrumbs' ), 20, 2 );
				add_filter( 'prisma_core_post_type_archive_title', array( $this, 'breadcrumb_post_type_archive_title' ), 10, 2 );

				// Remove WooCommerce sidebar.
				remove_action( 'woocommerce_sidebar', 'woocommerce_get_sidebar' );
				add_action( 'prisma_core_woocommerce_sidebar', 'woocommerce_get_sidebar' );
				...
				// Replace WooCommerce pagination with Prisma Core pagination.
				remove_action( 'woocommerce_after_shop_loop', 'woocommerce_pagination', 10 );
				add_action( 'woocommerce_after_shop_loop', 'prisma_core_pagination' );

				// Add back to shop button to Empty Cart.
				add_action( 'woocommerce_cart_is_empty', 'prisma_core_wc_empty_cart_button' );
				...
				add_filter( 'woocommerce_sale_flash', 'prisma_core_wc_add_percentage_to_sale_badge', 20, 3 );

				// Out of stock product badge.
				add_action( 'woocommerce_before_shop_loop_item_title', 'prisma_core_wc_out_of_stock_badge', 10 );
				add_action( 'woocommerce_before_single_product_summary', 'prisma_core_wc_out_of_stock_badge', 10 );
				...
				add_action( 'woocommerce_widget_shopping_cart_buttons', 'prisma_core_wc_widget_shopping_cart_buttons', 10 );
				...
				add_filter( 'woocommerce_subcategory_count_html', 'prisma_core_wc_cat_count_filter', 10, 2 );
				add_filter( 'woocommerce_rating_filter_count', 'prisma_core_wc_rating_count_filter', 10, 3 );
				add_filter( 'woocommerce_layered_nav_count', 'prisma_core_wc_layered_count_filter', 10, 3 );
				...
				// Add pr-entry class to single product summary (replaces template override).
				add_filter( 'woocommerce_post_class', array( $this, 'single_product_post_class' ), 10, 2 );
			}

			// Enqueue styles.
			add_action( 'prisma_core_enqueue_scripts', array( $this, 'enqueue' ) );
			...
			// Add correct sidebar.
			add_filter( 'prisma_core_sidebar_name', array( $this, 'set_sidebar' ) );

			// Set sidebar position.
			add_filter( 'prisma_core_default_sidebar_position', array( $this, 'set_default_sidebar_position' ) );

			// Remove item from cart.
			add_action( 'wp_ajax_prisma-core_remove_wc_cart_item', array( $this, 'remove_item_from_cart' ) );
			add_action( 'wp_ajax_nopriv_prisma-core_remove_wc_cart_item', array( $this, 'remove_item_from_cart' ) );

			// Add theme supports.
			add_action( 'after_setup_theme', array( $this, 'theme_supports' ), 20 );

			// Add customizer cart widget.
			add_filter( 'prisma_core_customizer_widgets', array( $this, 'add_customizer_cart_widget' ) );
			add_filter( 'prisma_core_main_header_widgets', array( $this, 'add_cart_to_main_header_widgets' ) );
			...
			// Add dynamic CSS.
			add_filter( 'prisma_core_dynamic_styles', array( $this, 'dynamic_css' ), 5 );

			// Update dynamic styles on deactivation.
			add_action( 'deactivate_woocommerce/woocommerce.php', array( prisma_core_dynamic_styles(), 'delete_dynamic_file' ) );

			// Return Shop page ID.
			add_filter( 'prisma_core_get_the_id', array( $this, 'get_the_id' ) );
		...
		public function theme_supports() {
			...
			if ( prisma_core_option( 'wc_product_gallery_zoom' ) ) {
				add_theme_support( 'wc-product-gallery-zoom' );
			}

			// Product Gallery Lightbox.
			if ( prisma_core_option( 'wc_product_gallery_lightbox' ) ) {
		...
		public function enqueue() {
			...
			wp_enqueue_style(
				'prisma-core-woocommerce',
				PRISMA_CORE_THEME_URI . '/assets/css/compatibility/woocommerce' . $prisma_core_suffix . '.css',
				false,
			...
			wp_enqueue_script(
				'prisma-core-wc',
				PRISMA_CORE_THEME_URI . '/assets/js/' . $prisma_core_dir . 'prisma-core-wc' . $prisma_core_suffix . '.js',
				array( 'jquery' ),
				PRISMA_CORE_THEME_VERSION,
				true
			);
		...
		public function product_catalog_elements() {
			...
			$elements = prisma_core_option( 'product_catalog_elements' );
		...
		public function template_loop_category() {
			...
			if ( is_array( $product_cats ) && ! empty( $product_cats ) ) {
				foreach ( $product_cats as $product_cat ) {
					$cats[] = '<a class="pr-loop-product__category" href="' . esc_url( get_term_link( $product_cat, 'product_cat' ) ) . '">' . esc_html( $product_cat->name ) . '</a>';
				}
			}

			echo '<span class="pr-loop-product__category-wrap">' . wp_kses_post( implode( ', ', $cats ) ) . '</span>';
		}
		...
		public function result_wrapper_start() {
			...
			echo '<div class="pr-woo-before-shop clearfix">';
		}
		...
		public function cart_widget_count_fragment( $fragments ) {
			...
			$fragments['.pr-header-widget__cart a.pr-cart'] = prisma_core_wc_cart_icon( false );
		...
		public function cart_widget_dropdown_fragment( $fragments ) {
			...
			$fragments['.pr-header-widget__cart .dropdown-item'] = prisma_core_wc_cart_dropdown( false );
		...
		public function content_wrapper_end() {
			?>
				...
				<?php do_action( 'prisma_core_woocommerce_sidebar' ); ?>
		...
		public function single_product_wrapper_start() {
			echo '<div class="pr-wc-product-wrap">';
		}
		...
		public function single_product_wrapper_end() {
			echo '</div><!-- END .pr-wc-product-wrap -->';
		}
		...
		public function product_actions() {
			...
			// Remove Prisma Core page title on WooCommerce product pages.
			add_filter( 'prisma_core_page_header_has_title', '__return_false' );

			// Disable Comments toggle.
			add_filter( 'prisma_core_display_comments_toggle', '__return_false' );
		}
		...
		public function add_customizer_cart_widget( $widgets ) {
			...
			$widgets['cart'] = 'Prisma_Core_Customizer_Widget_Cart';
		...
		public function register_wc_sidebars() {
			...
			register_sidebar(
				apply_filters(
					'prisma_core_woocommerce_sidebar_name',
					array(
						'name'          => esc_html__( 'WooCommerce Sidebar', 'prisma-core' ),
						'id'            => 'prisma-core-wc-sidebar',
						'description'   => __( 'Widgets in this area are displayed on WooCommerce pages except Product pages.', 'prisma-core' ),
						'before_widget' => '<div id="%1$s" class="pr-sidebar-widget pr-widget widget %2$s">',
						'after_widget'  => '</div>',
			...
			register_sidebar(
				apply_filters(
					'prisma_core_woocommerce_product_sidebar_name',
					array(
						'name'          => esc_html__( 'Product Sidebar', 'prisma-core' ),
						'id'            => 'prisma-core-wc-product-sidebar',
						'description'   => __( 'Widgets in this area are displayed on WooCommerce Product pages.', 'prisma-core' ),
						'before_widget' => '<div id="%1$s" class="pr-sidebar-widget pr-widget widget %2$s">',
						'after_widget'  => '</div>',
						'before_title'  => '<h4 class="widget-title">',
						'after_title'   => '</h4>',
		...
		public function set_sidebar( $sidebar_name ) {
			...
			if ( is_product() ) {
				$sidebar_name = 'prisma-core-wc-product-sidebar';
			} elseif ( is_woocommerce() || is_cart() || is_checkout() ) {
				$sidebar_name = 'prisma-core-wc-sidebar';
			}
		...
		public function set_default_sidebar_position( $position ) {
			...
			if ( is_product() ) {
				$position = prisma_core_option( 'wc_product_sidebar_position' );
			} elseif ( is_woocommerce() || is_cart() || is_checkout() ) {
				$position = prisma_core_option( 'wc_sidebar_position' );
			}

			if ( is_product() || is_woocommerce() || is_cart() || is_checkout() ) {
				if ( 'default' === $position ) {
					return prisma_core_option( 'sidebar_position' );
				} else {
					return $position;
				}
			}
		...
		/**
		 * Remove Prisma Core page description on WooCommerce pages.
		 *
		 * @since 1.0.0
		...
		public function remove_item_from_cart() {
			...
			check_ajax_referer( 'prisma-core-nonce' );
		...
		public function product_image_swap() {
			...
			$hover_style = prisma_core_option( 'shop_product_hover' );
			...
			if ( 'image-swap' === $hover_style ) {
				...
				if ( $attachment_ids ) {
					...
					echo wp_kses_post( apply_filters( 'prisma_core_woocommerce_product_image_swap', wp_get_attachment_image( $attachment_id, $image_size, false, array( 'class' => 'show-on-hover' ) ) ) );
				}
			}
		}
		...
		public function loop_product_thumb_wrap_start() {
			...
			if ( 'image-swap' === prisma_core_option( 'shop_product_hover' ) ) {
		...
		public function load_customizer_widget() {
			...
			$path = PRISMA_CORE_THEME_PATH . '/inc/compatibility/woocommerce/class-prisma-core-customizer-widget-cart.php';
		...
		public function review_order_heading() {
			...
			echo wp_kses_post( apply_filters( 'prisma_core_review_order_heading', '<h3>' . __( 'Payment', 'prisma-core' ) . '</h3>' ) );
		}
		...
		public function checkout_shipping_heading() {
			...
			echo wp_kses_post( apply_filters( 'prisma_core_checkout_shipping_heading', '<h3>' . __( 'Shipping', 'prisma-core' ) . '</h3>' ) );
		}
		...
		public function single_product_related_products_args( $args ) {
			...
			$columns = intval( prisma_core_option( 'wc_related_columns' ) );
			$rows    = intval( prisma_core_option( 'wc_related_rows' ) );
		...
		public function woocommerce_cross_sell_display() {
			...
			if ( prisma_core_option( 'wc_cross_sell_products' ) ) {

				$rows = intval( prisma_core_option( 'wc_cross_sell_rows' ) );
		...
		public function woocommerce_upsell_display() {
			...
			if ( prisma_core_option( 'wc_upsell_products' ) ) {

				$columns = intval( prisma_core_option( 'wc_upsells_columns' ) );
				$rows    = intval( prisma_core_option( 'wc_upsells_rows' ) );
		...
		public function woocommerce_related_products() {
			...
			if ( prisma_core_option( 'wc_related_products' ) ) {
		...
		public function single_product_slider_options( $options ) {
			...
			if ( ! prisma_core_option( 'wc_product_slider_arrows' ) ) {
				return $options;
			}

			$options['directionNav'] = true;
			$options['prevText']     = prisma_core_animated_arrow( 'left', false );
			$options['nextText']     = prisma_core_animated_arrow( 'right', false );
		...
		public function cross_sells_heading( $heading ) {
			return __( 'You may be interested in', 'prisma-core' );
		}

		/**
		 * Add pr-entry class to single product pages via woocommerce_post_class filter.
		 *
		 * @since  1.4.0
		...
		public function related_products_heading( $heading ) {
			return __( 'Related Products', 'prisma-core' );
		}
		...
		public function admin_redirects() {
			...
			if ( false !== strpos( $current_page, 'prisma-core' ) ) {
		...
		public function dynamic_css( $css ) {
			...
			$css .= '
				...
				.wc-layered-nav-rating a:hover .star-rating span:before {
					color: ' . prisma_core_luminance( $accent_color, .15 ) . ';
				}
			...
			$css .= '
				...
				.woocommerce .quantity .pr-woo-minus,
				.woocommerce .quantity .pr-woo-plus {
			...
			// Background.
			$header_background = prisma_core_option( 'header_background' );

			if ( 'color' === $header_background['background-type'] && $header_background['background-color'] ) {
				$css .= '
					.pr-header-widget__cart .pr-cart .pr-cart-count { 
			...
			// Headings.
			$css .= prisma_core_dynamic_styles()->get_typography_field_css( '.woocommerce div.product h1.product_title, .woocommerce #reviews #comments h2, .woocommerce .cart_totals h2, .woocommerce .cross-sells > h2, .woocommerce #reviews #respond .comment-reply-title', 'headings_font' );

			$css .= prisma_core_dynamic_styles()->get_typography_field_css( '.woocommerce div.product h1.product_title', 'h2_font' );
			$css .= prisma_core_dynamic_styles()->get_typography_field_css( '.woocommerce #reviews #comments h2', 'h3_font' );
			$css .= prisma_core_dynamic_styles()->get_typography_field_css( '.woocommerce .cart_totals h2, .woocommerce .cross-sells > h2, .woocommerce #reviews #respond .comment-reply-title', 'h4_font' );
...
if ( ! function_exists( 'prisma_core_woocommerce' ) ) :
	/**
	 * The function which returns the one Prisma_Core_Woocommerce instance.
	 *
	 * @since 1.0.0
	 * @return object
	 */
	function prisma_core_woocommerce() {
		return Prisma_Core_Woocommerce::instance();
	}
endif;

prisma_core_woocommerce();
