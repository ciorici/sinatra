<?php
/**
 * Prisma Core compatibility class for Elementor.
 *
 * @package Prisma Core
...
if ( ! class_exists( 'Prisma_Core_Elementor' ) ) :

	/**
	 * Elementor compatibility.
	 */
	class Prisma_Core_Elementor {
		 ...
		 *
		 * @since 1.0.0
		 * @return Prisma_Core_Elementor
		 */
		public static function instance() {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Prisma_Core_Elementor ) ) {
				self::$instance = new Prisma_Core_Elementor();
			}
		...
		public function enqueue_editor_style() {
			...
			wp_enqueue_style(
				'prisma-core-elementor-editor',
				PRISMA_CORE_THEME_URI . '/assets/css/compatibility/elementor-editor-style' . $prisma_core_suffix . '.css',
				false,
		...
		public function setup_postdata() {
			...
			if ( ! prisma_core_enable_page_builder_compatibility() ) {
				return;
			}
			...
			$id = prisma_core_get_the_id();

			$setup = get_post_meta( $id, '_prisma-core_page_builder_setup', true );

			if ( isset( $post ) && empty( $setup ) && ( is_admin() || is_singular() ) && empty( $post->post_content ) && $this->is_built_with_elementor( $id ) ) {

				update_post_meta( $id, '_prisma-core_page_builder_setup', true );
				update_post_meta( $id, 'prisma_core_disable_page_title', true );
				update_post_meta( $id, 'prisma_core_disable_breadcrumbs', true );
				update_post_meta( $id, 'prisma_core_disable_thumbnail', true );
				update_post_meta( $id, 'prisma_core_sidebar_position', 'no-sidebar' );

				update_post_meta( $id, '_wp_page_template', 'page-templates/template-prisma-core-fullwidth.php' );
			}
		...
		public function enqueue_additional() {
			...
			wp_enqueue_style(
				'prisma-core-elementor',
				PRISMA_CORE_THEME_URI . '/assets/css/compatibility/elementor' . $prisma_core_suffix . '.css',
				false,
				PRISMA_CORE_THEME_VERSION,
				'all'
			);
...
/**
 * Returns the one Prisma_Core_Elementor instance.
 */
function prisma_core_elementor() {
	return Prisma_Core_Elementor::instance();
}

prisma_core_elementor();
