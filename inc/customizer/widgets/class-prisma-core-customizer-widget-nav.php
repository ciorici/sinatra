...
if ( ! class_exists( 'Prisma_Core_Customizer_Widget_Nav' ) ) :

	/**
	 * Prisma Core Customizer widget class
	 */
	class Prisma_Core_Customizer_Widget_Nav extends Prisma_Core_Customizer_Widget {
		...
		public function __construct( $args = array() ) {
			...
			$this->name        = __( 'Navigation', 'prisma-core' );
			$this->description = __( 'Add a navigation menu.', 'prisma-core' );
		...
		public function form() {
			...
			$has_menu_class = ! empty( $menus ) ? 'prisma-core-widget-nav-has-menu' : '';
			?>
			<div class="prisma-core-widget-nav-container <?php echo esc_attr( $has_menu_class ); ?>">
				<p class="prisma-core-widget-nav-selected">
					<label for="widget-nav-<?php echo esc_attr( $this->id ); ?>-<?php echo esc_attr( $this->number ); ?>-menu">
						<?php echo esc_html_x( 'Select Menu:', 'Widget', 'prisma-core' ); ?>
					</label>
					...
					<select id="widget-nav-<?php echo esc_attr( $this->id ); ?>-<?php echo esc_attr( $this->number ); ?>-menu" name="widget-nav-<?php echo esc_attr( $this->number ); ?>-menu" data-option-name="menu">
						<option value="0"><?php echo esc_html_x( '&mdash; Select &mdash;', 'Widget', 'prisma-core' ); ?></option>
				...
				<p class="prisma-core-widget-nav-empty">
					<?php
					printf(
						/* Translators: %1$s - anchor tag start. %2$s anchor tag end. */
						esc_html__( 'No menus found: %1$sCreate a new menu?%2$s', 'prisma-core' ),
