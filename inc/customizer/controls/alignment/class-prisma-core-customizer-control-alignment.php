...
if ( ! class_exists( 'Prisma_Core_Customizer_Control_Alignment' ) ) :
	/**
	 * Prisma Core Customizer custom background control class.
	 */
	class Prisma_Core_Customizer_Control_Alignment extends Prisma_Core_Customizer_Control {
		 ...
		 * @var string
		 */
		public $type = 'prisma-core-alignment';
		...
		public function __construct( $manager, $id, $args = array() ) {
			...
			$this->strings = wp_parse_args(
				...
				array(
					'top-left'     => __( 'Top Left', 'prisma-core' ),
					'top'          => __( 'Top', 'prisma-core' ),
					'top-right'    => __( 'Top Right', 'prisma-core' ),
					'left'         => __( 'Left', 'prisma-core' ),
					'center'       => __( 'Center', 'prisma-core' ),
					'right'        => __( 'Right', 'prisma-core' ),
					'bottom-left'  => __( 'Bottom Left', 'prisma-core' ),
					'bottom'       => __( 'Bottom', 'prisma-core' ),
					'bottom-right' => __( 'Bottom Right', 'prisma-core' ),
				)
			);
		...
		protected function content_template() {
			?>
			<div class="prisma-core-alignment-wrapper prisma-core-control-wrapper">
				...
				<div class="prisma-core-alignment-control">

					<# if ( ! data.choices['top-left'] && ! data.choices['top'] && ! data.choices['top-right'] ) { #>
						<div class="button-group pr-top">
							<label class="pr-left {{{ data.choices['top-left']}}}">
							...
							<label class="pr-center {{{ data.choices['top']}}}">
							...
							<label class="pr-right {{{ data.choices['top-right']}}}">
					...
					<# if ( ! data.choices['left'] && ! data.choices['center'] && ! data.choices['right'] ) { #>
						<div class="button-group pr-middle">
							<label class="pr-left {{{ data.choices['left']}}}">
							...
							<label class="pr-center {{{ data.choices['center']}}}">
							...
							<label class="pr-right {{{ data.choices['right']}}}">
					...
					<# if ( ! data.choices['bottom-left'] && ! data.choices['bottom'] && ! data.choices['bottom-right'] ) { #>							
						<div class="button-group pr-bottom">
							<label class="pr-left {{{ data.choices['bottom-left']}}}">
							...
							<label class="pr-center {{{ data.choices['bottom']}}}">
							...
							<label class="pr-right {{{ data.choices['bottom-right']}}}">
