<?php
/**
 * Prisma Core Customizer Widget control class.
 *
 * @package     Prisma Core
...
if ( ! class_exists( 'Prisma_Core_Customizer_Control_Widget' ) ) :
	/**
	 * Prisma Core Customizer custom select control class.
	 */
	class Prisma_Core_Customizer_Control_Widget extends Prisma_Core_Customizer_Control {
		 ...
		 * @var string
		 */
		public $type = 'prisma-core-widget';
		...
		public function to_json() {
			...
			if ( is_array( $this->widgets ) && ! empty( $this->widgets ) ) {
				...
				foreach ( $this->widgets as $widget_id => $args ) {
					...
					ob_start();
					?>
					<div data-widget-id="<?php echo esc_attr( $instance->id_base ); ?>" class="prisma-core-widget-tpl-<?php echo esc_attr( $instance->id_base ); ?> prisma-core-widget" data-max-uses=<?php echo esc_attr( $max_uses ); ?>>
		...
		protected function content_template() {
			?>
			<div class="prisma-core-control-wrapper prisma-core-widget-wrapper">

				<div class="prisma-core-widget-container">

					<# _.each( data.value, function( template, i ) { #>
						{{{ template }}}
					<# }); #>

				</div><!-- END .prisma-core-widget-container -->

				<div class="prisma-core-add-widget-wrap">			
					<button type="button" class="button prisma-core-add-widget" data-location-title="{{{ data.label }}}" data-control="{{{ data.id }}}"><?php esc_html_e( 'Add Widget', 'prisma-core' ); ?></button>
				</div>

				<div class="prisma-core-widget-tmpls">
