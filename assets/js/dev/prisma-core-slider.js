/**
 * Prisma Core hover slider
 *
 * @since 1.0.0
 */
var prismaCoreHoverSlider = function(el) {

	var current = 0,
		spinner = el.querySelector( '.pr-spinner' );
	...
	imagesLoaded( el.querySelectorAll( '.hover-slider-backgrounds' ), { background: '.hover-slide-bg' }, function() {
		...
		if ( null !== preloader && ! document.body.classList.contains( 'pr-loaded' ) ) {
			document.body.addEventListener( 'pr-preloader-done', function() {
				setTimeout( function() {
					hideSpinner();
				}, 300);
			});
	...
	el.querySelectorAll( '.hover-slider-item-wrapper' ).forEach( ( item ) => {
		item.addEventListener( 'mouseenter', function() {
			...
			if ( current !== prismaCoreGetIndex(item) ) {
				current = prismaCoreGetIndex(item);
...
( function() {
	...
	document.addEventListener( 'DOMContentLoaded', function () {
		...
		// Init sliders
		document.querySelectorAll( '.pr-hover-slider' ).forEach( ( item ) => {
